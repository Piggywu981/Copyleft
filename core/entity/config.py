...
class Config(object):
    ...
    def set_quality(self, quality):
        """设置图片质量"""
        self._data['base']['quality'] = quality
    ...
    def get_layout_type(self) -> str:
        """获取布局类型，如果为null则返回默认布局类型"""
        layout_type = self._data['layout']['type']
        # 如果布局类型为null或不存在，返回默认布局类型
        if layout_type is None or layout_type == 'null':
            return 'watermark_left_logo'  # 默认布局类型
        return layout_type
    ...
    def get_element_name(self, location) -> str:
        """获取指定位置的元素名称，如果为null或缺失则返回默认值"""
        try:
            element_name = self._data['layout']['elements'][location]['name']
            if element_name is None or element_name == 'null':
                return 'None'  # 默认返回值
            return element_name
        except (KeyError, TypeError):
            return 'None'  # 如果键不存在或数据结构有问题，返回默认值
